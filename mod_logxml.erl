...
filter(FilterO, E) ->
    {{orientation, OrientationO},{stanza, StanzaO},{direction, DirectionO}} = FilterO,
        ?DEBUG("FilterO: ~p",[FilterO]),
    {Orientation, From, To, Packet} = E,
    ?DEBUG("E: ~p",[E]),
    ?DEBUG("Packet before: ~p",[Packet]),
    ?DEBUG("Stanza_str before: ~p",[Packet#xmlel.name]),
...
send_packet(FromJID, ToJID, P) ->
    ...
    Proc = gen_mod:get_module_proc(Host, ?PROCNAME),
    ?DEBUG("send_packet Proc: ~p",[Proc]),
    ?DEBUG("send_packet FromJID: ~p",[FromJID]),
    ?DEBUG("send_packet ToJID: ~p",[ToJID]),
    ?DEBUG("send_packet P: ~p",[P]),
